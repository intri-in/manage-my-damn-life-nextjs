...
class DashboardView extends Component {
    ...
    async eventDrop(e) {
        ...
        var eventData = this.state.allEvents[newID]
        if (this.state.allEvents && this.state.allEvents[newID] && this.state.allEvents[newID].type != "VTODO" && this.state.allEvents[newID].type != "VTIMEZONE" && varNotEmpty(this.state.allEvents[newID])) {
    ...
    async eventResize(e) {
        console.log("eventResize", this.state.allEvents[e.event.id])
        var newID = e.event.id
        if (varNotEmpty(this.state.allEvents[e.event.id]) == false) {
            //Probably a recurring event. Get ID from map.
            newID = this.state.recurMap[e.event.id]
        ...
        //Calculate delta in ms
        var eventData = _.cloneDeep(this.state.allEvents[newID])
        if (this.state.allEvents && this.state.allEvents[newID] && this.state.allEvents[newID].type != "VTODO" && this.state.allEvents[newID].type != "VTIMEZONE" && varNotEmpty(this.state.allEvents[newID])) {
